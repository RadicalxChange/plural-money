      ...
      <div className="w-full px-24 pb-24 pt-12">
        ...
        <ul className="w-full max-w-5xl font-mono text-sm border-b border-gray-300 bg-gradient-to-b from-zinc-200 pb-6 backdrop-blur-2xl dark:border-neutral-800 dark:bg-zinc-800/30 dark:from-inherit rounded-xl border bg-gray-200 lg:p-4">
          {transactions.reverse().map((transaction, index) => {
            return (
              <li key={index} className={`px-4 py-2 my-2 flex justify-between items-center ${transaction.sender_id === user.id ? 'text-red-400' : ''} ${transaction.recipient_id === user.id ? 'text-green-400' : ''}`}>
                  {getName(transaction.sender_id)} sent {transaction.amount} Whocoins to {getName(transaction.recipient_id)} for {transaction.message}
              </li>
            )
          })}
        </ul>
